...
describe('gain command', () => {
  ...
  it('shows suggestion stats from stored suggestions', async () => {
    ...
    storeSuggestions([
      { id: randomUUID(), target: 'claude_md', scope: 'global', rule: 'Rule A', confidence: 'high' as const, reasoning: 'R', friction_types: ['wrong_approach' as const], status: 'applied', source_analysis: 'a-1' },
      { id: randomUUID(), target: 'claude_md', scope: 'global', rule: 'Rule B', confidence: 'high' as const, reasoning: 'R', friction_types: ['wrong_approach' as const], status: 'applied', source_analysis: 'a-2' },
      { id: randomUUID(), target: 'skill', scope: 'project', rule: 'Rule C', confidence: 'medium' as const, reasoning: 'R', friction_types: ['wrong_approach' as const], status: 'pending', source_analysis: 'a-3' },
    ]);
